    ...
    while True:
        ...
        if not drop:
            ...
            if Gram is None:
                ...
                L[n_active, :len(active)] = np.dot(X.T[n_active], X.T[:len(active)].T)
            else:
                ...
                L[n_active, :len(active)] = Gram[n_active, :len(active)]

            # Update the cholesky decomposition for the Gram matrix
            if n_active:
                linalg.solve_triangular(
                    L[:len(active), :len(active)],
                    L[n_active, :len(active)],
                    trans=0,
                    lower=1,
                    overwrite_b=True,
            ...
            v = np.dot(L[n_active, :len(active)], L[n_active, :len(active)])
        ...
        least_squares, _ = solve_cholesky(
            L[:len(active), :len(active)], sign_active[:len(active)], lower=True
        )

        if least_squares.size == 1 and least_squares == 0:
            # This happens because sign_active[:len(active)] = 0
            least_squares[...] = 1
            AA = 1.0
        else:
            # is this really needed ?
            AA = 1.0 / np.sqrt(np.sum(least_squares * sign_active[:len(active)]))

            if not np.isfinite(AA):
                # L is too ill-conditioned
                i = 0
                L_ = L[:len(active), :len(active)].copy()
                while not np.isfinite(AA):
                    L_.flat[:: n_active + 1] += (2 ** i) * eps
                    least_squares, _ = solve_cholesky(
                        L_, sign_active[:len(active)], lower=True
                    )
                    tmp = max(np.sum(least_squares * sign_active[:len(active)]), eps)
                    AA = 1.0 / np.sqrt(tmp)
                    i += 1
            least_squares *= AA
        ...
        if Gram is None:
            ...
            eq_dir = np.dot(X.T[:len(active)].T, least_squares)
            ...
            corr_eq_dir = np.dot(Gram[:len(active), n_active:].T, least_squares)
        ...
        if drop and method == "lasso":
            ...
            for ii in idx:
                arrayfuncs.cholesky_delete(L[:len(active), :len(active)], ii)
            ...
            if True:
                ...
                residual = y - np.dot(X[:, :len(active)], coef[active])
