...
def load_featstruct(phi_directory,X_directory,R_directory,K_directory,n_rois,n_features,slices):
    ...
    n_cases = len(X_directory_struct)
    print(n_cases)
    print(len(phi_directory_struct))
    X_all = np.full((n_cases,n_rois,n_features),0)
    R_all = np.full((n_cases,n_rois,n_features),0).astype(str)
    K_all = np.full((n_cases,n_rois,n_features),0).astype(str)
    # Load feature arrays
    count = 0
    r_count = 0
    print('Allocated arrays')
    print(len(X_directory_struct))
    if slices == True:
        for feature_array in X_directory_struct:
            X_case = np.load(X_directory+feature_array,allow_pickle=True)
            print(feature_array[4:6])
            ID_all.append(feature_array[4:6])
            ...
            slice_roi = np.arange(len(np.unique(R_case)))
            all_slice_roi.append(len(slice_roi))
            if len(slice_roi) > 0:
                print(np.unique(R_case))
                print(len(np.unique(R_case)))
                print('Populating matrix with',X_directory+feature_array,'of shape',str(X_case.shape),'with',str(len(slice_roi)),
                      'ROIs at slice',str(count),'with',str(feature_roi),'and',str(feature_key))
                X_case_rs = X_case.reshape((len(slice_roi),int(X_case.shape[0]/len(slice_roi))))
                for j in np.arange(len(slice_roi)):
                    X_all[count,int(slice_roi[j]),:] = X_case_rs[j,:]
            else:
                print('No unique ROIs?',R_case)
                r_count = r_count+1
            count = count+1
        print(stats.mode(np.asarray(all_slice_roi)))
        print(count)
        print(r_count)
    if slices == False:
        # Load features
...
def re_index(X_all,K_all,R_all,c_cases_idx,subs,ids,all_rois,pre_imp,pre_updrs_off,post_imp):
    ...
    if all_rois == True:
        ...
        R = R_all_c.reshape((R_all_c.shape[0],R_all_c.shape[1]*R_all_c.shape[2]))
    return X_all_c, K, R, subsc, pre_imp, pre_updrs_off, per_change
...
def make_feature_matrix(X_all_c,pre_metric,scaler,all_roi,slices):
    if all_roi == True:
...
def model_scale(scaler_type,X_train,train_index,X_test,test_index,pre_metric,all_roi,scale_together,slices):
    scaler = scaler_type
    if scale_together == True:
        X_all = np.vstack((X_train,X_test))
        pre_metric = np.hstack((pre_metric[train_index],pre_metric[test_index]))
        X0_tt,scaler = make_feature_matrix(X_all,pre_metric,scaler,all_roi,slices)
        X_test_in = X0_tt[test_index,:]
        X0_tt = X0_tt[train_index,:]
    else:
        X0_tt,scaler = make_feature_matrix(X_train,pre_metric[train_index],scaler,all_roi,slices)
        X_test_in = scale_feature_matrix(X_test,pre_metric[test_index],scaler,all_roi,slices)
    return X0_tt,scaler,X_test_in

def scale_feature_matrix(X_test,pre_metric_test,scaler,all_roi,slices):
    if all_roi == True:
