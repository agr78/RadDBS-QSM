...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.decode(z), mu, logvar, yh
    
    def predict(self,x):
        return self.r(F.relu(x))
...
def train_model(X_train,y_train,model,X_test):
    ...
    for epoch in range(num_epochs):
        ...
        for case in np.arange(X_train.shape[0]):
            ...
            optimizer.zero_grad()
            recon_batch, mu, logvar, yh = model(img)
            loss = 0.5*loss_function(recon_batch, img, mu, logvar)+0.9*mse(yh[0],y_train[case].cuda())
            loss.backward()
            train_loss += loss
            optimizer.step()
            # if epoch % 100 == 0:
