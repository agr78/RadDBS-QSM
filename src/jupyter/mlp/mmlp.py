...
class Net(nn.Module):
    def __init__(self,in_size,n_channels,n_samples,ks,rad_dim):
        ...
        self.relu = nn.ReLU()
        self.lifter = nn.Linear(in_size**2,in_size**2)
        self.encoder = resnet50(weights='DEFAULT')
        for param in self.encoder.parameters():
            param.requires_grad = False
        self.r = nn.Linear(1000+self.rad_dim,1)
  
    # Estimate Aw = b
    def encode(self, x):
        zt = self.relu(self.lifter(x.view(-1,1,self.in_size**2)))
        zt = torch.repeat_interleave(zt.cpu(), 3, dim=1).cuda()
        z = self.relu(self.encoder(zt.view(-1,3,self.in_size,self.in_size)))
        return z
    # Forward pass
...
def main(model,subsc,data_dir,subfolder,y,X,train_id,test_id,workers,num_epochs,lr,scheduled_decay,num_neighbors,batch_size,reg_type,alpha,aug_state,factor,save_state,verbose):
    ...
    if aug_state == False:
        ...
        model_path = './trained_models/model_{}.pth'.format(np.asarray(subsc)[test_id][0])
