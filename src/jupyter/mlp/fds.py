...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, bucket_num, bucket_start, ks, sigma, start_smooth, start_update=0,
                 kernel='gaussian', momentum=0.9):
    ...
    def calibrate_mean_var(self,matrix, m1, v1, m2, v2, clip_min=0.1, clip_max=10):
            ...
            for label in torch.unique(labels):
                if label > self.bucket_num - 1 or label < self.bucket_start:
                    ...
                    features[labels == label] = self.calibrate_mean_var(
                        ...
                        self.smoothed_mean_last_epoch[int(label - self.bucket_start)],
                        self.smoothed_var_last_epoch[int(label - self.bucket_start)])
                #print('Smoothing label',label.item(),'at minimum bucket',self.bucket_start,'out of',self.bucket_num,'buckets')
        return features
