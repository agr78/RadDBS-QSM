...
class Net(nn.Module):
    def __init__(self,in_size,n_channels,ks,mp,rad_dim):
        ...
        self.relu = nn.ReLU()
        self.dp = nn.Dropout(p=0.1,inplace=False)
        self.sigmoid = nn.Sigmoid()
        # self.bn = nn.BatchNorm2d(self.n_channels, affine=False)
        self.conv2 = nn.Conv2d(in_channels=self.n_channels, out_channels=self.n_channels, kernel_size=self.ks, stride=1) 
        # self.conv3 = nn.Conv2d(in_channels=self.n_channels, out_channels=self.n_channels, kernel_size=self.ks, stride=1)
        # self.conv4 = nn.Conv2d(in_channels=self.n_channels, out_channels=self.n_channels, kernel_size=self.ks, stride=1)
        # self.conv5 = nn.Conv2d(in_channels=self.n_channels, out_channels=self.n_channels, kernel_size=self.ks, stride=1)
        n_layers = 2
    ...
    def encode(self, x):
        ...
        #zc5 = self.dp(self.relu(self.conv5(zc4)))
        zc5 = zc2
...
def main(model,subsc,data_dir,subfolder,y,X,train_id,test_id,workers,num_epochs,lr,num_neighbors,batch_size,reg_type,alpha,aug_state,factor,save_state,verbose):
