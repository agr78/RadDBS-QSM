...
def train_estimator(subsc,X_all_c,K_all_c,per_change,pre_updrs_off,age,sex,dd,ledd,Q,delta,aug,reg,save,rs0,verbose):
        ...
        if aug == 'nc_iid_q':
            for jj in np.arange(Q):
              ...
              while np.sum(y_cat) < cvn:
                ...
                y_train_n = y_train0
                X0_ss0_n = X0_ss00
              y_train_n = np.append(y_train_n,y_train0+(delta*sigma)*np.random.normal(0,1,1))
              y_cat = y_train_n <= 0.3
  ...
  if save == True:
      np.save('results'+'msw_'+str(est_type)+'_'+str(aug)+'_'+str(rs0)+'_cvs_puo.npy',results)
