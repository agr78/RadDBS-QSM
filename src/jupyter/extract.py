...
def feature_matrix(raddbs_path,segs,qsms,subsc,reextract):
    ...
    if reextract == 1:
        ...
        # Get all image types from PyRadiomics
        all_image_types = featureextractor.getImageTypes()
        # Enable all image types except 'lbp-2D'
        for img_type in all_image_types:
            if img_type != 'LBP2D':
                try:
                    extractor.enableImageTypeByName(img_type)
                except Exception as e:
                    print(f"Warning: Could not enable image type '{img_type}': {e}")
        ...
        for i in np.arange(subsc.__len__()):
            ...
            print('Extracting features for subject',subsc[i],'and appending feature matrix with vector of length',fv_count)
    ...
    if reextract == 0:
        ...
        K_all_c = K.reshape(n_cases,n_rois,n_features)[:,0:4,:]
        R_all_c = np.repeat(R[1:5],n_features)
