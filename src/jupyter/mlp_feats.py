...
class MLP(nn.Module):
    def __init__(self,in_size):
        ...
        self.r0 = nn.Linear(self.in_size, self.in_size//2)
        self.r1 = nn.Linear(self.in_size//2, self.in_size//2)
        self.r2 = nn.Linear(self.in_size+(self.in_size//2),1)
        self.relu = nn.ReLU(inplace=True)
    # Estimate Aw = b
    def encode(self, x):
        z = self.r1(self.relu(self.r0(x)))
        z = self.r2(self.relu(torch.hstack((x,z))))
        return z
    # Forward pass
    def forward(self, x):
        y = self.encode(x)
        ...
        for batch_case in np.arange(num_batches):
                ...
                if val_loss < best_loss:
                    ...
                    if verbose > 1:
                        print('Best validation loss:',str(val_loss.item()),
                            ...
                            'and epoch',epoch)
                    model_path = './trained_models/mlp/model_{}.pth'.format(case_id)
            ...
            # Training loss 
                ...
                if loss < best_loss:
                    ...
                    model_path = './trained_models/mlp/model_{}.pth'.format(case_id)
                    if warm_start_weights != []:
                        model_path = './trained_models/mlp/model_{}_{}.pth'.format(case_id,'alpha')
            ...
            for k in np.arange(len(lrs)):
                if warm_start_weights != []:
                    if j == 0:
                        ...
                        warm_start_weights = './trained_models/mlp/model_{}_{}.pth'.format(case_id,'alpha')
